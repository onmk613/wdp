...
| [07-内置模块手册](07-内置模块手册.md) | 22 个内置模块的参数、幂等机制、check/diff 行为与示例 |
| [08-Chart 应用包](08-chart应用包.md) | 目录结构、三层 values、子 chart、生命周期、脚本模块 |
...
| [14-代码阅读指南](14-代码阅读指南.md) | 面向开发者：架构分层、wdp run 全链路、包导读、关键机制、排错定位、上手练习 |
| [15-自治执行改造方案](15-自治执行改造方案.md) | 分期改造路线与设计依据：P0 地基修复、P1 plan 一等产物（`wdp plan`/`apply`）、P2 agent 自治执行（断连容忍/中继/断点续跑）；P0–P2 已实施 |
| [16-host-config 与结构自描述方案](16-host-config与结构自描述方案.md) | 设计记录：`--host-config` 内联主机连接配置（待实施）；`wdp schema` 结构字段自描述（host/task 已实施，对账测试防漂移） |

> 文档中的示例均可在 `examples/` 目录中找到对应可运行版本。
