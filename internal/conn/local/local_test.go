...
func TestLocalExec(t *testing.T) {
	...
	t.Run("echo 输出与退出码 0", func(t *testing.T) {
		res, err := l.Exec(ctx, conn.ExecRequest{Script: "echo hello"})
		if err != nil {
	...
	t.Run("非零退出码折叠进 Code", func(t *testing.T) {
		res, err := l.Exec(ctx, conn.ExecRequest{Script: "exit 3"})
		if err != nil {
	...
	t.Run("stderr 单独分流", func(t *testing.T) {
		res, err := l.Exec(ctx, conn.ExecRequest{Script: "echo err >&2"})
		if err != nil {
			t.Fatal(err)
		}
		if res.Stderr != "err\n" || res.Stdout != "" {
	...
	t.Run("stdin 传递", func(t *testing.T) {
		res, err := l.Exec(ctx, conn.ExecRequest{Script: "cat", Stdin: "abc"})
		if err != nil {
			t.Fatal(err)
		}
		if res.Stdout != "abc" {
	...
	t.Run("Env 设置", func(t *testing.T) {
		res, err := l.Exec(ctx, conn.ExecRequest{
...
func TestLocalConnectCloseHostname(t *testing.T) {
	...
	for i := range 2 { // Connect 幂等
		if err := l.Connect(ctx); err != nil {
			t.Fatalf("Connect 第 %d 次报错: %v", i+1, err)
		}
	}
	for i := range 2 { // Close 幂等
...
func TestLocalFactoryRegistered(t *testing.T) {
	c, err := conn.NewConnection(&model.Host{Name: "via-factory", Conn: "local"}, nil)
	if err != nil {
