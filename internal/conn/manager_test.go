...
func TestManagerConcurrentHandshake(t *testing.T) {
	...
	RegisterFactory("slowtest", func(h *model.Host, dc *Defaults) (Conn, error) {
		cur := inFlight.Add(1)
		for {
			m := maxInFlight.Load()
...
func TestManagerSameHostDedup(t *testing.T) {
	RegisterFactory("deduptest", func(h *model.Host, dc *Defaults) (Conn, error) {
		return &slowConn{name: h.Name}, nil
	})
...
func TestManagerClosedGetAfterClose(t *testing.T) {
	RegisterFactory("closedtest", func(h *model.Host, dc *Defaults) (Conn, error) {
		return &slowConn{name: h.Name}, nil
	})
	m := NewManager()
