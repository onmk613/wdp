...
func TestSaveLoadContainedInDir(t *testing.T) {
	...
	if id != "../evil-1700000000" {
		t.Fatalf("id = %q", id)
	}
	// 收敛后的落盘位置在 releases 内; 未越出到 ~/.wdp 之外
	if _, err := os.Stat(filepath.Join(relDir, "evil-1700000000.json")); err != nil {
		t.Errorf("记录应被收敛写入 releases 目录: %v", err)
	}
	if _, err := os.Stat(filepath.Join(home, "evil-1700000000.json")); err == nil {
		t.Errorf("记录越出了 releases 目录")
	}

	// 穿越写法的 Load 同样被收敛: 解析到同一文件而非目录外
	rec, err := Load("../evil-1700000000")
	if err != nil {
