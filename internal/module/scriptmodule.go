...
//
// 用户可见文档：chart 自带脚本模块——modules/<名> 可执行脚本，参数经
// WDP_MODULE_ARGS(JSON)/WDP_FREE_FORM 环境变量传入；stdout 输出
...
func RunScriptModule(rc *RunContext, path string, args map[string]any, free string) *Result {
	...
	if err != nil {
		return Fail(i18n.T("failed to read script module: %v", "读取脚本模块失败: %v"), err)
	}
	if rc.CheckMode {
		...
		if !rc.CheckScriptAllowed {
			return &Result{Skipped: true, Msg: fmt.Sprintf(i18n.T("[check] skipping script module (chart does not declare check_mode: supported): %s", "[check] 跳过脚本模块（chart 未声明 check_mode: supported）: %s"), path)}
		}
...
func runScriptModuleCode(rc *RunContext, data []byte, args map[string]any, free string, check bool) *Result {
	...
	if err != nil {
		return Fail(i18n.T("failed to serialize module arguments: %v", "模块参数序列化失败: %v"), err)
	}
	remote := "/tmp/.wdp-mod-" + tempSuffix()
	if err := rc.Conn.UploadFile(rc.Ctx, remote, bytes.NewReader(data), 0o755); err != nil {
		return Fail(i18n.T("failed to upload script module: %v", "上传脚本模块失败: %v"), err)
	}
	defer func() {
...
exit $rc`, shellquote.Quote(remote), shellquote.Quote(remote))
	...
	if out.Code != 0 && !res.Failed {
		res.Failed = true
		res.Msg = fmt.Sprintf(i18n.T("script module exit code rc=%d: %s", "脚本模块退出码 rc=%d: %s"), out.Code, firstLine(out.Stderr))
	}
	return res
}
