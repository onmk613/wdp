...
func RunScriptModule(rc *RunContext, path string, args map[string]any, free string) *Result {
	...
	if err != nil {
		return Fail("failed to read script module: %v", err)
	}
	if rc.CheckMode {
		...
		if !rc.CheckScriptAllowed {
			return &Result{Skipped: true, Msg: fmt.Sprintf("[check] skipping script module (chart does not declare check_mode: supported): %s", path)}
		}
...
exit $rc`, shellquote.Quote(remote), shellquote.Quote(remote))
	...
	if out.Code != 0 && !res.Failed {
		res.Failed = true
		res.Msg = fmt.Sprintf("script module exit code rc=%d: %s", out.Code, firstLine(out.Stderr))
	}
	return res
}
