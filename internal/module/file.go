...
func parseFileArgs(rc *RunContext, args map[string]any) (*fileReq, *Result) {
	...
	if !ok || path == "" {
		return nil, Fail("file 需要 path 参数（或 dest 别名）")
	}
	state, _ := argStr(args, "state")
	switch state {
	case "", "file", "directory", "link", "touch", "absent":
	default:
		return nil, Fail("不支持的 state %q（可选: file/directory/link/touch/absent）", state)
	}
	...
	if (fr.owner != "" || fr.group != "") && !rc.Become {
		return nil, Fail("设置 owner/group 需要 become: true（%s）", fr.path)
	}
	if fr.state == "link" && fr.src == "" {
		return nil, Fail("state=link 需要 src 指定链接目标")
	}
	return fr, nil
}
...
func (m *FileModule) Run(rc *RunContext, args map[string]any, free string) *Result {
	...
	if fr.state == "file" && kind == "missing" {
		return Fail("文件不存在: %s（state=file 只校验不创建）", fr.path)
	}
...
func convergeFileState(rc *RunContext, fr *fileReq, kind string, ch *fileChanges) *Result {
	...
	case fr.state == "link":
		...
		if cur != fr.src {
			if rc.CheckMode {
				ch.add(fmt.Sprintf("将链接 → %s", fr.src))
			} else {
				if rc.Rollback != nil {
					if kind == "missing" {
						rc.Rollback.RecordRemove(fr.path)
				...
				ch.add(fmt.Sprintf("已链接 → %s", fr.src))
			}
		}
	}
	return nil
...
func fixFileAttrs(rc *RunContext, fr *fileReq, kind string, ch *fileChanges) *Result {
	if kind == "missing" && !ch.changed {
		...
		return &Result{Msg: fmt.Sprintf("%s 不存在，无属性可校正", fr.path)}
	}
	if fr.hasMode {
		...
		if rc.CheckMode {
			if cur, ok, mbad := remoteMode(rc, fr.path); mbad != nil {
				...
				ch.add(fmt.Sprintf("权限 → %04o", wantMode),
	...
	if fr.owner != "" || fr.group != "" {
		...
		if !ok || curOwner != fr.owner || curGroup != fr.group {
			if rc.CheckMode {
				ch.add(fmt.Sprintf("属主 → %s:%s", fr.owner, fr.group),
